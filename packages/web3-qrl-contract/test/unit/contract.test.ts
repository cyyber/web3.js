...
import {
	AllGetPastEventsData,
	getLogsData,
	getPastEventsData,
	GREETING_CHANGED_TOPIC,
} from '../fixtures/unitTestFixtures';
...
const GREETER_DEPLOYMENT_DATA = `${GreeterBytecode}${encodeParameter('string', 'My Greeting').slice(2)}`;
...
describe('Contract', () => {
	describe('constructor', () => {
		...
		it('should init with abi and address', () => {
			const contract = new Contract([], TEST_ADDRESS_1);
		...
		it('should init with abi, options and context', () => {
			const contract = new Contract(
				[],
				{ gas: '123' },
				{ config: { defaultAccount: TEST_ADDRESS_1 } },
			);
		...
		it('should pass the returnDataFormat to `_parseAndSetAddress` and `_parseAndSetJsonInterface`', () => {
			...
			contract.options.address = TEST_ADDRESS_5;
	...
	describe('Contract functions and defaults', () => {
		...
		const deployedAddr = TEST_ADDRESS_4;
		...
		it('defaults set and get should work', () => {
			...
			const defaultAddr = TEST_ADDRESS_6;
		...
		it('should be able to call a payable method', async () => {
			...
			const spyQRLCall = jest
				...
				.mockImplementation(async (_objInstance, _tx) => {
					expect(_tx.to).toBe(TEST_ADDRESS_2);
					expect(_tx.input).toBe(
						'',
					);
					return '0x00';
				});

			await expect(
				contract.methods.approve(TEST_ADDRESS_1, 1).call(),
			).resolves.toBeTruthy();
		...
		it('contract method send without from address should throw error', async () => {
			...
			const contract = new Contract(GreeterAbi);
			contract.options.address = TEST_ADDRESS_3;
		...
		it('contract method createAccessList should work', async () => {
			const fromAddr: Address = TEST_ADDRESS_4;
