...
describe('format', () => {
	...
	describe('methods', () => {
		it.each(Object.values(FMT_NUMBER))('getBlockNumber', async format => {
			web3QRL.defaultReturnFormat = { number: format as FMT_NUMBER, bytes: FMT_BYTES.HEX };
		...
		it.each(Object.values(FMT_NUMBER))('getBalance', async format => {
			...
			const res = await web3QRL.getBalance(newAccount.address);
			expect(typeof res).toBe(mapFormatToType[format as string]);
			expect(numberToHex(res)).toBe(value);
		});

		it.each(Object.values(FMT_BYTES))('getCode', async format => {
			web3QRL.defaultReturnFormat = { number: FMT_NUMBER.BIGINT, bytes: format };
			const code = await web3QRL.getCode(contractDeployed?.options?.address as string);
